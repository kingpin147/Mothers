...
export const journalPost = pgTable(
  "journal_post",
  {
    ...
    slug: text("slug").notNull().unique(),
    category: text("category").default("postpartum").notNull(),
    ...
    author: text("author").default("The Mothers").notNull(),
    authorRoleEn: text("author_role_en"),
    authorRoleEs: text("author_role_es"),
    bylineEn: text("byline_en"),
    bylineEs: text("byline_es"),
    reviewedNoteEn: text("reviewed_note_en"),
    reviewedNoteEs: text("reviewed_note_es"),
    heroImageId: text("hero_image_id"),
    audience: text("audience").default("public").notNull(),
    status: text("status").default("draft").notNull(), // 'draft', 'scheduled', 'published', 'unpublished'
    publishedAt: timestamp("published_at", { withTimezone: true }),
